...
Our area of interest is a large rectangular area in southern British Columbia containing the burn boundary from the 2017 Elephant Hill wildfire. The fire began on July 6th, 2017 and burned 191,865 ha in the south-central Interior region of BC, just northeast of Cache Creek (BC Gov News, 2020). It was the largest fire by area in the record-breaking 2017 BC wildfire season and was likely started by smoking materials (matches, cigarettes, etc.) (CBC News, 2020).

```{r 01-elephant-hill-fire, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-elephant-hill-fire.png")
```
...
-   Train, test, and predict a continuous variable (crown closure) using the random forest algorithm

-   Train, test, and predict a categorical variable (fuel type) using the random forest algorithm
...
<input type="checkbox" unchecked> Answers to the questions posed throughout the lab (X points)</input> <input type="checkbox" unchecked> A PDF map of ONE of the FUEL_TYPE rasters and ONE of the CROWN_CLOS rasters, complete with all the necessary elements (labels, scale, compass, legend, etc.). These should be a full page each and separate files from your report. Be sure to specify in a caption or title what sampling design / parameter settings you are displaying in each.</input> <input type="checkbox" unchecked> A PDF report discussing the following points (max 5 pages, not including figures):</input>

-   What are the results of the spatial autocorrelation (SA) tool and what do they tell us? Why did we use the K Nearest Neighbours method for this dataset? Display the VRI dataset and observe the distribution of the points across the landscape. What is the main difference between this method and distance-based methods? Why is accounting for SA in your data important? If you were not limited by the tool inputs, how would you change your sampling design to account for SA?
-   What variables did you choose to include in your model after running exploratory regression? Why did you choose this equation and/or set of variables? Use the statistical terms in the results (R2, p-value, AIC, VIF, variable significance, etc.) to help you explain.
...
##### Q1. What is the null hypothesis used in this tool? (1 point) {.unnumbered}

##### Q2. Why did we test CROWN_CLOS for SA but not FUEL_TYPE? (1 point) {.unnumbered}
...
```{r 01-exploratory-regression, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-exploratory-regression.png")
```

##### Q3. What is the maximum R2 value out of all the equations in the results? (1 point)
...
##### Q4. How many fuel types are there? (1 point) {.unnumbered}

##### Q5. Which fuel type has the fewest number of points? (1 point) {.unnumbered}
...
```{r 01-elephant-hill-fishnet, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-elephant-hill-fishnet.png")
```

**Step 8:** Open the "Feature to Point" tool and use your fishnet from the previous step as the input. The default of this tool when used with polygon geometry is to place a point at the center of each polygon. If you overlay your new point feature class onto your fishnet, it should look like the screenshot below.

```{r 01-elephant-hill-fishnet-points, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-elephant-hill-fishnet-points.png")
```
...
```{r 01-spatial-join, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-spatial-join.png")
```
...
Our last task will be using the "Forest-based Classification and Regression" tool to run our machine learning. ESRI provides a helpful overview of the tool [here](https://pro.arcgis.com/en/pro-app/latest/tool-reference/spatial-statistics/how-forest-works.htm). We will be running it a total of nine times (three with FUEL_TYPE, six with CROWN_CLOS) and it can be slow, so please be patient and leave yourself lots of time.

**Step 1:** Open the "Forest-based Classification and Regression" tool. When you search for the tool, check that it is from the "Spatial Statistics" toolbox and NOT the "GeoAnalytics Desktop" toolbox as they have different inputs. Parameterize the tool as follows:
...
For CROWN_CLOS, instead of changing the sampling design, we are going to experiment with different parameter settings on the tool for a total of 6 runs (three different parameters, two settings each).

**Step 3:** Open the "Forest-based Classification and Regression" tool again and set these inputs which will act as our control:
