...
**Responses to the questions posed throughout the lab on the course management system (25 points)**

**Q1. How many fuel types are in the VRI dataset? Which fuel type has the fewest points? (3 points)**

**Q2. What are the results of the spatial autocorrelation (SA) analysis? What do they indicate? Why was SA tested for CROWN_CLOS but not for FUEL_TYPE? (4 points)**

**Q3. Which model performed best for FUEL_TYPE and for CROWN_CLOS? Briefly justify your choice (5 points)**

**Q4. How did model performance differ among the three sampling strategies for fuel type? What are the main advantages and limitations of each? (3 points)**

**Q5. How did Random Forest parameter changes affect crown closure model performance? Which parameters should be optimized and why? (4 points)**

**Q6. Why were the selected variables effective for predicting crown closure? What other variables might better predict fuel type? (3 points)**

**Q7. What are the strengths and limitations of using remote sensing, climate, and terrain data for predicting fuel types and crown closure? (3 points)**



**Map with Predictive Fuel types and Crown Closure (15 points)**

**A summary table of the Exploratory Regression diagnostics (R², AIC, VIF, significance) for the three best models. Which model had the highest R² and which variables were selected for the final model? (10 points)**
...
|                                                              | Source: ClimateNA                                                                                                               |                 |
+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------+-----------------+
| Summer Heat Moisture Index (SHM)                             | $\frac{mean\text{ }warmest\text{ }month\text{ }temperature}{\frac{May\text{ }to\text{ }September\text{ }precipitation}{1000} }$ | CNA_SHM        |
|                                                              |                                                                                                                                 |                 |
|                                                              | Source: ClimateNA                                                                                                               |                 |
+--------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------+-----------------+
...
**HINT: Due to the constraint of completing this lab in ArcGIS Pro, we are unable adjust our sampling parameters to account for spatial autocorrelation. Fortunately, this is a learning exercise, so you will have the opportunity to discuss this conundrum when interpreting your results!**
...
**Step 6:** Select the equation you think will work best for "CROWN_CLOS". Remember to look at all the different measures of model "goodness" (R^2^, AIC, VIF, multicollinearity, etc.). Our variable rasters fall into three categories: terrain, spectral, and climate. Our final equation should include at least one variable from each category. Write down the variables from your chosen equation somewhere as you will have to re-enter them later.

##### Q1. How many fuel types are in the VRI dataset? (2 points) {.unnumbered}

##### Q2. What are the results of the spatial autocorrelation (SA) analysis? What do they indicate? Why was SA tested for CROWN_CLOS but not for FUEL_TYPE? (4 points) {.unnumbered}

##### Table 1. Create a summary table of the Exploratory Regression diagnostics (R², AIC, VIF, significance) for the three best models. (5 points) {.unnumbered}

##### Q3. Which model had the highest R² and which variables were selected for the final model? (4 points) {.unnumbered}
...
Our last task will be using the "Forest-based and Boosted Classification and Regression" tool to run our machine learning. ESRI provides a helpful overview of the tool [here](https://pro.arcgis.com/en/pro-app/latest/tool-reference/spatial-statistics/how-forest-works.htm). 
...
**HINT: Under the "Analysis" tab, view your tool "History". Double-click the "Forest-based and Boosted Classification and Regression" tool and it will pop up in your geoprocessing pane with the same inputs and settings you used for that particular run -- this way, you can just edit the inputs that are changing between runs rather than having to reenter the settings each time.**
...
Open the "Forest-based and Boosted Classification and Regression" tool again and set these inputs which will act as our control:
...
**Step 4:** We are going to run the "Forest-based and Boosted Classification and Regression" tool six more times. For each time, the inputs should be identical to those in Step 3 except for a single parameter which we will change each time:
