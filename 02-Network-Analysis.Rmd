...
For this lab, you will identify the preferred habitat for salmon spawning based on proxy measures and how it corresponds to our stream network that we have derived from the DEM. This lab will walk you through how to measure stream order and slope. For this lab, you will also be expected to work with one Conservation Unit.

We will focus on Vancouver Island for a few reasons. First, islands make it possible to calculate flow accumulation exactly, which is important for creating a (nearly) topologically correct stream network. Second, Vancouver Island is large, which will enable us to look at natural sources of topological errors. Third, many species of Pacific salmon return to highly developed Vancouver Island to spawn and often come into conflict with human development.
...
<input type="checkbox" unchecked> Responses to the questions posed throughout the lab on the course management system (25 points)  

<input type="checkbox" unchecked> Map panel (PDF or image) (15 points) 
Submit a panel of maps for **one conservation unit only**, including:

- Stream routes symbolized by reachability from the ocean 
- Dams represented as barriers  
- Accessible vs inaccessible salmon habitat segments  
- A zoomed-in map highlighting Lake Cowichan  
- Basic map elements (title, legend, scale bar)

<<<<<<< HEAD
<input type="checkbox" unchecked> Summary table (PDF, image, or text) (10 points)  
=======
---

<input type="checkbox" unchecked> **Summary table of your Conservation Unit (PDF, image, or text) (10 points)**  

>>>>>>> 2be541b99e41de67b5194354c84ec95fc183ca0e

- Total number of accessible Streams
- Total number of inaccessible Streams
- Total number of dam junctions 
- Total number of ocean junctions
...
1.  Download all the tiles for Vancouver Island from the B.C. Government [here](https://https://catalogue.data.gov.bc.ca/)
...
**c) Salmon conservation units (CU):**
...
Right click on the DEM layer in the Layers pane >  Export > Save as. Save the file as a GeoTiff in your Lab 2 project folder as **vancouver_island_dem**. Leave the default settings. Close QGIS. 

**Step 2:** Go back to ArcGIS Pro, and make sure you are connected to the salmon database on the UBC PostgreSQL server and locate the **salmon.public.vancouver_island_boundary** layer. Once you find it, add it to your project. Also, add the Vancouver Island DEM (the one you just exported from QGIS) to your map project. Then, clip the DEM using salmon.public.vancouver_island_boundary with the Clip Raster tool, and make sure to toggle on “Use input features for clipping geometry.” 

```{r 02-vancouver_island_clip, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_clip.png")
```
...
Calculate the flow direction of the DEM, using the "Flow Direction" tool. The flow direction tool determines which raster cells flow into other cells based on the elevation difference the immediate neighborhood. Keep the defaults of the tool. Ensure that "Force all edge cells to flow outward" is toggled on and leave all other parameters with the default value. Save the output as **vancouver_island_flow_direction**. The cell values in the output raster represent the steepest down slope direction of each pixel. 

```{r 02-vancouver_island_flow_direction, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_flow_direction.png")
```

**Step 3:** Use the output from the "Flow Direction" tool as an input into the "Flow Accumulation" tool. Again, use the defaults within the tool, but select integer as the output data type because the output of this tool is a count. Save it as **vancouver_island_flow_accumulation** This tool will create a raster that represents the number of pixels that will accumulate into any given pixel. You may need to zoom into the output to be able to see these streams. Play around with the symbology to visualize the stream network. (Hint: try using classify and various methods within symbology).

```{r 02-vancouver_island_flow_accumulation, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_flow_accumulation.png")
```


**Step 4:** To define streams, you need to set a threshold for the flow accumulation. Open "Con"(Image Analyst Tools). This tool allows you to set a true/false condition and assign values to a new raster based on whether the condition is met. Set the expression so that a pixel that has a count equal to or more than 1000 will be assigned a value of 1. For "Input true raster or constant value", use value 1. For "Input false raster or constant value", use value 0. Save it as **vancouver_island_con**.

```{r 02-vancouver_island_con, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_con.png")
```
...
##### Q1. What is the maximum number of stream orders observed on Vancouver Island? (2 points)**
...
**Step 2:** Convert your stream features into routes using the "Create routes" tool. Use you Stream Features output as the Input Line. Set the Route ID field to "arcid". The coordinate Priority parameter can be set to any corner. When you go to save your output, navigate into your feature dataset (it "feels" like entering another folder from the Catalog view), and save the output as "vancouver_island_stream_routes".

```{r 02-vancouver_island_stream_routes, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_stream_routes.png")
```

**Step 3:** Right-click on the feature dataset, select "Import", then Feature Class(es). Navigate to the **salmon.publi.dams** layer on the UBC PostgreSQL server, and use it as Input Features. Click Run and import this layer into your feature dataset.
...
```{r 02-create-topology-1, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/02-create_topology_wizard.png")
```
...
```{r 02-create-topology-2, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/02-create_topology_wizard-2.png")
```
...
```{r 02-topology-errors, out.width= "25%", echo = FALSE}
    knitr::include_graphics("images/02-create_topology_errors.png")
```
...
**Step 9:** Take the Vancouver Island boundary polygon provided and perform a negative buffer, called a nibble. Open the Buffet tool, and use -25 meters for the buffer distance. We are doing a negative buffer because we want to ensure that this edge intersects with the streams that terminate at the ocean. Remember that the streams were generated by an imperfect and imprecise DEM, therefore this process will ensure that every stream that reaches the ocean will intersect this boundary.

```{r 02-vancouver_island_nibble, out.width= "25%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_nibble.png")
```
...
##### Q2. How many dams junctions are present in your selected conservation unit? (1 point)**

##### Q3. How many ocean junctions are present in your selected conservation unit? (1 point)**
...
**Step 1:** Calculate the slope of the clipped DEM using "Percent rise" as the output measurement and "Planar" as the Method. 
...
```{r 02-locate-features-along-routes, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_stream_routes_gradient.png")
```
...
```{r 02-stream-routes-gradient, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/02-vancouver_island_stream_routes_gradient_table.png")
```
...
**Step 6:** Repeat steps 4 and 5 above for the stream order raster that you generated in Task 1. Ensure that the stream_order raster is used as the "Input stream raster" and "Simplify polygons" is toggled off. In the end, you should have a second event table where the grid_code indicates the stream order for each route. Again, add a Short field to the event table called "SORDER" and calculate this field as !grid_code! to transfer the grid_code values to a more meaningful field name.

##### Q4. How would the stream network change if a 5 m DEM were used instead of a 100 m DEM? (7 points)**

<<<<<<< HEAD
##### Q5. How can unfilled sinks and topological errors in the DEM affect the modeled stream network? (7 points)**
=======
**Q6: When zooming into Lake Cowichan, what artifact is observed in the stream network and why does it occur? (7 points)**
>>>>>>> 2be541b99e41de67b5194354c84ec95fc183ca0e
...
```{r 02-create-trace-network, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-create_trace_network.png")
```
...
```{r 02-traced-network, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/02-traced_network.png")
```
...
```{r 02-overlay-route-events, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-overlay_route_events.png")
```
...
```{r 02-join-field, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-join_field.png")
```
...
```{r 02-stream-route-overlay, out.width= "100%", echo = FALSE}
    knitr::include_graphics("images/02-stream_route_overlay.png")
```
...
**For your final map, you will clip these layers to one of the salmon conservation units.**
