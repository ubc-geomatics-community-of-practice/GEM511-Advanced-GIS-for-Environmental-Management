...
**Step 2:** Now in ArcGIS Pro, add the Vancouver Island DEM to your map project. 
...
**Step 1:** Convert your stream features into routes using the "Create routes" tool. Set the Route ID field to "arcid". The coordinate Priority parameter can be set to any corner. Save the output as "vancouver_island_stream_routes".

```{r 02-create-routes, out.width= "50%", echo = FALSE}
    knitr::include_graphics("images/02-create-routes.png")
```

**Step 2:** Create a blank feature dataset and name it "stream_routes_dams". To do this, right-click the geodatabase in the Catalog pane and select New Feature Dataset. For the projection, use NAD 1983 BC Environment Albers (EPSG: 3005).

**Step 3:** Click and drag the stream routes and dams feature classes to the new feature dataset within the Catalog view. 
...
The following steps will use the "stream_dams_junction", "stream_ocean_junctions" and the "vancouver_island_stream_routes" layers to trace stream routes starting from the ocean and are unimpeded by dams. You will need to save them in the Topology geodatabase. 

Before running the next steps save your ArcGIS Project. Ensure you have cleared any selected features from the map. 
