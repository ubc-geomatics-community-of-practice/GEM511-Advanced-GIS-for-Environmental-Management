...
The tool above is the most straight-forward way to create a DEM from a LiDAR point cloud in ArcGIS Pro. In the following steps you will create a 2D point dataset using a random subset of the LiDAR point cloud. This will allow us to test and compare three interpolation methods by comparing the interpolated surfaces to the DEM fit using the entire liDAR point cloud. 
...
**Step 5:** Search for the "LAS to Multipoint" tool. Input your LAS Dataset. You will notice that there is a box asking for the "Average Point Spacing". Enter the value that you recorded from Step 3. Set the correct coordinate system and then run the tool.
...
ArcGIS Pro will attempt to draw the symbols for every single point in the multipart and singlepart feature classes, so keep them toggled off.
...
**Step 8:** We will take a random 70% subset of points from the new singlepoint feature class. First, open the attribute table and create a new numeric **"Float"** field called "random_nums". We will use the "Calculate Field" tool to populate the field with random numbers. 

The screenshot shows how to set up the Calculate Field window using a Python expression in the Code Block. You will have more practice using this functionality in future labs! Note that because there are almost 3 million points these steps will take several minutes to complete. Use this opportunity to hydrate and stretch :) 

```{r 01-random, out.width= "75%", echo = FALSE}
    knitr::include_graphics("images/01-random.png")
```

Next, sort the table in Descending order and select ~30% of the rows to delete. As selecting exactly 30% of the rows is quite finicky, don't worry about getting the exact right number but be sure to report how many features were removed and how many remained in the methods section of your report!

We now have a 2D point feature class that we can use to test different spatial interpolation algorithms! 

It is important to save your project at this step so you don't lose any outputs before continuing. 
