...
# Spatial interpolation and visualization of LiDAR Data {#terrain-spatial-interpolation}
...
<input type="checkbox" unchecked> Table that shows zonal statistics of the difference rasters across elevation and slope ranges of the binned DEM (10 points)</input>

<input type="checkbox" unchecked> A panel of maps that symbolizes the DEMs produced and the difference rasters that were produced by the comparison to the binning approach (15 points)</input>
...
[Pipelines](https://pdal.io/en/stable/pipeline.html#pipelines) are JSON-formatted files that give PDAL a set of instructions for reading, processing and writing point cloud data. With pipelines, you can define every step of the processing that you want and you can specify the finest level of detail at every stage. Pipelines are executed linearly, so we read the instructions from top-down. Below is an example of a pipeline that we are going to use, which is described in more detail below:
...
This will thin the point cloud by selecting a random point within a sphere with 5 m radius. The output will be called "thinned.las" and will be written to the current working directory.
...
**Step 1:** Right-click the **mkrf_aoi_lidar** layer, open "Properties", select "Symbology" from the left, and then select "Attribute by Ramp" from the very top drop-down menu. Then choose your attribute and apply your symbology parameters.
...
if (knitr:::is_latex_output()) {
  knitr::include_graphics("images/01-qgis-dock-3d-map-view.png")
} else {
...
if (knitr:::is_latex_output()) {
  knitr::include_graphics("images/01-qgis-shift-up-down.png")
} else {
...
```{r 01-qgis-shift, out.width= "100%", echo = FALSE}
if (knitr:::is_latex_output()) {
  knitr::include_graphics("images/01-qgis-shift.png")
} else {
...
```{r 01-qgis-ctrl, out.width= "100%", echo = FALSE}
if (knitr:::is_latex_output()) {
  knitr::include_graphics("images/01-qgis-ctrl.png")
} else {
...
```{r 01-qgis-alt, out.width= "100%", echo = FALSE}
if (knitr:::is_latex_output()) {
  knitr::include_graphics("images/01-qgis-alt.png")
} else {
...
```{r 01-qgis-scroll, out.width= "100%", echo = FALSE}
if (knitr:::is_latex_output()) {
  knitr::include_graphics("images/01-qgis-scroll.png")
} else {
  knitr::include_graphics("images/01-qgis-scroll.gif")
}
```

**Step 4:** Practice navigating in 3D and observing the different attributes across the ground surface. There is a button on the top of the 3D map view to save an image of any given view.
...
##### Q2. What differences do you observe between the thinned and non-thinned datasets in terms of number of points, average point spacing, and the measured range of elevation? (3 points) {.unnumbered}
...
##### Q3. How might changing the parameters of this tool ("Interpolation Type", "Cell Assignment", and "Sampling Value") affect the statistical qualities of the output DEM? (6 points) {.unnumbered}
...
**Step 1:** Create three rasters for each interpolation method: Natural Neighbor, IDW, and Spline. We suggest writing these rasters out to GeoTiffs so that you can also view them in QGIS, if you want. For each tool, keep the default settings, but make sure that you generate the rasters at 1 m resolution. Note that the Spline tool will take several minutes to run. If you have trouble running this step try generating the rasters at a slightly coarser resolution (5 m). 
...
Next, we are going to quantitatively evaluate the differences and calculate statistics over different elevation and slope classes to examine how the interpolation methods perform.

**Step 3:** Change the Symbology of the **MKRF_DEM** to Classify and set the number of Classes to 3 with the method of your choice. Use the "Reclassify (Spatial Analyst)" tool to reclassify the binned **MKRF_DEM** into "high", "medium", and "low" areas using this classification scheme. (If you symbolize first, then open the Reclassify tool it should populate the start and end fields with the symbolized values.)
...
##### Q4. Summarize and describe how the interpolation methods differ from the binned DEM across elevation and slope classes. Which zones/methods are most similar or dissimilar? (10 points) {.unnumbered}

##### Table that shows zonal statistics of the difference rasters across elevation and slope ranges of the binned DEM (10 points) {.unnumbered}

**Step 6:** Create a final map with the following elements: 
...
- North arrow and appropriate labels for each map panel

##### A panel of maps that symbolizes the DEMs produced and the difference rasters that were produced by the comparison to the binning approach (15 points) {.unnumbered}
...
##### Q5. Which spatial interpolation algorithm would you recommend the research forest use? Defend and justify your choice. (5 points) {.unnumbered}

------------------------------------------------------------------------

## Task 5: Visualize 3D data in ArcGIS Pro {.unnumbered}

In this last brief task are some instructions for visualizing the point cloud and interpolated DEM surfaces in ArcGIS Pro.
...
if (knitr:::is_latex_output()) {
  knitr::include_graphics("images/01-arcgis-scene-navigation.png")
} else {
